...
export function CreateWorkoutScreen({
  ...
  const handleSaveWorkout = (
    ...
    onSaveWorkout(workoutName, exerciseData, restData);
    // Reset form
    setExercises([]);
    setRestPeriods([]);
  };
  ...
  const removeRest = (id: string) => {
    setRestPeriods(
      restPeriods.filter((rest) => rest.id !== id),
    );
  };
    ...
    <div className="flex-1 p-4 pb-20 bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen">
      <div className="max-w-md mx-auto space-y-6">
        ...
        {exercises.map((exercise) => (
          <Card
            key={exercise.id}
            className="p-4 bg-white/80 backdrop-blur-sm border-0 shadow-md"
          >
        ...
        {exercises.length === 0 && restPeriods.length === 0 && (
          <Card className="p-8 text-center bg-white/80 backdrop-blur-sm border-0 shadow-md">
            ...
            <p className="text-slate-500 text-sm">
              Add exercises and rest periods to create your
